...
private actor DownloadCancellationStore {
...
extension RequestManager {
    ...
    private func downloadFile(destinationURL: URL,
                              ...
                              onTaskCreated: (@Sendable (URLSessionDownloadTask) -> Void)? = nil) {
        Task {
    ...
    private func downloadFileWithRequest(request: URLRequest,
                                         ...
                                         completion: (@Sendable (Result<Int, Error>) -> Void)? = nil,
                                         progress: (@Sendable (Float) -> Void)? = nil,
                                         onTaskCreated: (@Sendable (URLSessionDownloadTask) -> Void)? = nil) -> URLSessionDownloadTask? {
    ...
    public func download(destinationURL: URL,
        ...
        let cancellationStore = DownloadCancellationStore()
        return try await withTaskCancellationHandler {
            try await withCheckedThrowingContinuation { continuation in
                self.downloadFile(destinationURL: destinationURL,
                    ...
                    Task {
                        await cancellationStore.setTask(task)
                    }
                })
            }
        } onCancel: {
